...
//http://ideone.com/BhmiJt
//http://ideone.com/s7GCFD

//#define DEBUG

inline bool compareBySum(const tri& t1, const tri& t2) { return t1.a < t2.a; }
tri MaxIntersectingSubarray(int *a, int lo, int hi) {
    int mid = (lo + hi)/2;
    int Lsum = 0, l = mid+1;
    int sum = 0;
    ...
    return {Lsum+Rsum, l, r};
}
tri MaxSubarraySum(int* a, int lo, int hi) {
    if (lo == hi) {
        return {a[lo], lo, hi};
    } else {
        int mid = (lo + hi)/2;
        tri L = MaxSubarraySum(a, lo, mid);
        tri R = MaxSubarraySum(a, mid+1, hi);
        tri M = MaxIntersectingSubarray(a, lo, hi);
        return max({L, M, R}, compareBySum);
    }
}
